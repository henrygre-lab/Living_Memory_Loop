 ...
 * Passes through formats already accepted by transcription APIs so ffmpeg
 * is only needed as a fallback when format is unknown or OGG conversion is desired.
 */
export async function ensureCompatibleFormat(
  ...
  if (detected === "m4a") return { buffer: audioBuffer, format: "m4a" };
  ...
  if (detected === "ogg") {
    try {
      ...
      return { buffer: audioBuffer, format: "ogg" };
    }
  }
  ...
  try {
    ...
    throw new Error(
      "Unsupported audio format and ffmpeg conversion failed. Send wav/mp3/webm/mp4/m4a audio or install ffmpeg."
    );
  }
}
